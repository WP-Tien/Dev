...
function segmentHTMLToShortcodeBlock(HTML) {
  ...
  const transformation = findTransform(transformsFrom, transform => excludedBlockNames.indexOf(transform.blockName) === -1 && transform.type === 'shortcode' && some(castArray(transform.tag), tag => regexp(tag).test(HTML)));
  ...
  if (match = next(transformTag, HTML, lastIndex)) {
    ...
    if (typeof transformation.transform === 'function') {
      ...
      const attributes = mapValues(pickBy(transformation.attributes, schema => schema.shortcode), // Passing all of `match` as second argument is intentionally broad
      // but shouldn't be too relied upon.
      //
      // See: https://github.com/WordPress/gutenberg/pull/3610#discussion_r152546926
