...
export function getInnerBlocksProps() {
  ...
  const children = createElement(RawHTML, null, html);
  return { ...props,
    children
  };
}
...
export function getSaveElement(blockTypeOrName, attributes) {
  ...
  if (element !== null && typeof element === 'object' && hasFilter('blocks.getSaveContent.extraProps') && !(blockType.apiVersion > 1)) {
     ...
     * @param {Object}  attributes Block attributes.
     */
    const props = applyFilters('blocks.getSaveContent.extraProps', { ...element.props
    }, blockType, attributes);
...
export function getCommentDelimitedContent(rawBlockName, attributes, content) {
  const serializedAttributes = !isEmpty(attributes) ? serializeAttributes(attributes) + ' ' : ''; // Strip core blocks of their namespace prefix.
...
export default function serialize(blocks, options) {
  return castArray(blocks).map(block => serializeBlock(block, options)).join('\n\n');
}
